  ...
  return (
    <header
      className={`fixed top-0 inset-x-0 z-50 transition-all duration-300 ${
        scrolled ? "bg-white/95 backdrop-blur border-b border-black/8" : "bg-transparent"
      }`}
    >
      <div className="max-w-[1440px] mx-auto px-6 lg:px-12 h-16 lg:h-20 flex items-center justify-between">
          ...
          {NAV.map((n) => (
            <a
              ...
              className={`relative text-[14px] tracking-wide transition-colors ${
                n.active ? "text-black" : "text-black/70 hover:text-black"
              } group`}
            >
              {n.label}
              <span
                className={`absolute -bottom-1.5 left-0 h-px bg-[#E11D2E] transition-all duration-300 ${
        ...
        <div className="hidden lg:flex items-center gap-5">
          ...
          <a
            href="#contact"
            className="group flex items-center gap-2 px-5 py-2.5 bg-black text-white text-[13px] tracking-wide hover:bg-[#E11D2E] transition-colors"
          >
            Liên hệ kinh doanh
