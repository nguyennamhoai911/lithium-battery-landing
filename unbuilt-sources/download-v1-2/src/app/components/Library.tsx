...
import { Search, X, ChevronDown, Download, Eye, Link2, FileText, List, Rows3, Table2, Loader2, ArrowRight } from "lucide-react";
...
export function Library() {
  ...
  return (
    <section id="library" className="relative py-24 lg:py-32 bg-white border-t-2 border-[#0A0A0A]">
      <div className="max-w-[1440px] mx-auto px-6 lg:px-12">
        <div className="grid lg:grid-cols-12 gap-10 mb-12">
          <div className="lg:col-span-7">
            <SectionLabel num="03" label="THƯ VIỆN TÀI LIỆU" />
            <SectionHeading>
              Toàn bộ tài liệu<br />
              <span className="text-[#E60023]">kỹ thuật</span>
            </SectionHeading>
            <p className="text-[16px] leading-[1.6] text-black/70 mt-6 max-w-xl">
              Duyệt tài liệu chính thức của PKG Battery theo loại sản phẩm, điện áp, ứng dụng và danh mục tài liệu.
            </p>
          </div>
          <div className="lg:col-span-5 flex flex-wrap items-end gap-3 lg:justify-end">
            <div className="flex items-center border-2 border-[#0A0A0A] px-4 py-3 bg-white min-w-[260px] flex-1 lg:flex-none">
              <Search className="w-4 h-4 text-[#0A0A0A] mr-2" strokeWidth={2.5} />
              <input placeholder="Tìm trong kết quả…" className="bg-transparent outline-none text-[13px] flex-1 placeholder:text-black/45" style={{ fontWeight: 500 }} />
            </div>
            <button className="flex items-center gap-2 border-2 border-[#0A0A0A]/15 px-4 py-3 text-[13px] hover:border-[#0A0A0A]" style={{ fontWeight: 600 }}>
              Mới cập nhật <ChevronDown className="w-4 h-4" />
            </button>
            <div className="hidden md:flex border-2 border-[#0A0A0A]">
              {[
                ...
                <button
                  ...
                  className={`relative px-3.5 py-3 transition-colors ${view === v.k ? "bg-[#0A0A0A] text-white" : "text-black hover:bg-[#FAFAFA]"}`}
                  aria-label={v.label}
                >
                  <v.icon className="w-4 h-4" strokeWidth={2.2} />
                </button>
              ))}
            </div>
            <button onClick={() => setShowSheet(true)} className="lg:hidden border-2 border-[#0A0A0A] px-4 py-3 text-[13px]" style={{ fontWeight: 600 }}>Bộ lọc</button>
          </div>
        </div>
        ...
        <div className="grid lg:grid-cols-12 gap-10">
          ...
          <div className="lg:col-span-9">
            {/* Active filter chips */}
            <div className="flex flex-wrap items-center gap-2 mb-6 pb-5 border-b-2 border-[#0A0A0A]">
              <span className="text-[14px] text-[#0A0A0A] mr-2" style={{ fontWeight: 700 }}>
                {loading ? "Đang cập nhật…" : `${DOCS.length} tài liệu`}
              </span>
              {active.map((a) => (
                <button key={a} onClick={() => toggle(a)} className="group flex items-center gap-2 px-3 py-1.5 bg-[#0A0A0A] text-white text-[12px] tracking-wide hover:bg-[#E60023]" style={{ fontWeight: 600 }}>
                  {a}
                  <X className="w-3 h-3" strokeWidth={2.5} />
                </button>
              ))}
              {active.length > 0 && (
                <button onClick={() => setActive([])} className="text-[12px] text-[#E60023] underline-offset-4 underline ml-1" style={{ fontWeight: 700 }}>
                  Xóa tất cả
                </button>
              )}
            </div>
            ...
            {loading ? (
              ...
              <div className="divide-y-2 divide-[#0A0A0A]/8 border-b-2 border-[#0A0A0A]">
                {DOCS.map((d) => (
                  <DocRow key={d.id} d={d} />
                ))}
              </div>
...
function DocRow({ d }: { d: Doc }) {
  return (
    <div className="group relative flex flex-col lg:flex-row gap-6 px-4 py-7 transition-colors hover:bg-[#FAFAFA]">
      <div className="absolute left-0 top-0 bottom-0 w-1 bg-[#E60023] origin-top scale-y-0 group-hover:scale-y-100 transition-transform duration-300" />
      <div className="lg:w-36 flex flex-row lg:flex-col items-start gap-2 shrink-0">
        <span className="text-[10px] tracking-[0.25em] px-2.5 py-1.5 bg-[#0A0A0A] text-white group-hover:bg-[#E60023] transition-colors" style={{ fontWeight: 700 }}>
          {d.label}
        </span>
        <span className="text-[11px] text-black/55 lg:mt-1 tracking-wide" style={{ fontWeight: 600 }}>{d.fileType}</span>
      </div>
      <div className="flex-1 min-w-0">
        <h3 className="text-[20px] lg:text-[22px] tracking-[-0.01em] leading-[1.25] mb-3 group-hover:text-[#E60023] transition-colors" style={{ fontWeight: 700 }}>
          {d.title}
        </h3>
        <p className="text-[14.5px] leading-[1.6] text-black/70 max-w-3xl mb-4">{d.desc}</p>
        <div className="flex flex-wrap gap-1.5 mb-4">
          {d.tags.map((t) => (
            <span key={t} className="text-[11.5px] tracking-wide text-black/75 px-2 py-0.5 bg-black/[0.06]" style={{ fontWeight: 600 }}>
              {t}
            </span>
          ))}
        </div>
        <div className="text-[12px] text-black/55 tracking-wide flex flex-wrap gap-x-3 gap-y-1" style={{ fontWeight: 500 }}>
          <span>{d.fileType}</span>
          <span className="text-black/25">·</span>
          <span>{d.size}</span>
          <span className="text-black/25">·</span>
          <span>Cập nhật {d.updated}</span>
          <span className="text-black/25">·</span>
          <span className="text-[#E60023]" style={{ fontWeight: 700 }}>v{d.version}</span>
        </div>
      </div>
      <div className="flex flex-row lg:flex-col gap-2 lg:items-end shrink-0 lg:w-56">
        <button className="relative flex items-center justify-center gap-2 px-5 py-3 bg-[#0A0A0A] text-white text-[13px] tracking-wide hover:bg-[#E60023] transition-colors flex-1 lg:flex-none lg:w-full" style={{ fontWeight: 700 }}>
          <Download className="w-4 h-4" strokeWidth={2.5} />
          {d.primaryAction}
        </button>
        <div className="flex gap-2 lg:w-full">
          <button className="flex-1 flex items-center justify-center gap-2 border-2 border-[#0A0A0A] px-3 py-2.5 text-[12px] hover:bg-[#0A0A0A] hover:text-white transition-colors" style={{ fontWeight: 600 }}>
            <Eye className="w-3.5 h-3.5" strokeWidth={2.2} /> {d.secondaryAction}
          </button>
          <button className="border-2 border-[#0A0A0A] px-3 py-2.5 hover:bg-[#0A0A0A] hover:text-white transition-colors" aria-label="Sao chép liên kết">
